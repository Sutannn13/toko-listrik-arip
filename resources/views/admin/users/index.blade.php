...
@section('content')
    ...
    <div class="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <div
            class="rounded-2xl border border-green-200 bg-green-50 p-5 shadow-sm dark:border-green-500/20 dark:bg-green-500/10">
        ...
        <div
            class="rounded-2xl border border-error-200 bg-error-50 p-5 shadow-sm dark:border-error-500/20 dark:bg-error-500/10">
            <p class="text-xs font-semibold uppercase tracking-wider text-error-600 dark:text-error-400">Disuspend</p>
            <p class="mt-2 text-3xl font-black text-error-700 dark:text-error-400">{{ number_format($counts['suspended']) }}
            </p>
        </div>
    </div>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-dark-border dark:bg-dark-card"
        ...
        <p class="px-4 pt-3 text-xs font-medium text-gray-500 sm:hidden">Geser tabel ke samping untuk melihat kolom aksi.
        </p>
                        ...
                        <tr class="group transition hover:bg-gray-50/50 dark:hover:bg-dark-hover/50">
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-gradient-to-br from-brand-400 to-brand-600 text-xs font-bold text-white">
                                    ...
                                    <div>
                                        <p class="text-sm font-semibold text-gray-800 dark:text-white">
                                            ...
                                            @if ($isSelf)
                                                <span
                                                    class="ml-1 rounded bg-brand-100 px-1.5 py-0.5 text-[10px] font-bold text-brand-600 dark:bg-brand-500/10 dark:text-brand-400">KAMU</span>
                                            @endif
                                        </p>
                            ...
                            <td class="px-5 py-4">
                                @if ($user->is_suspended)
                                    <span
                                        class="inline-flex items-center gap-1 rounded-full bg-error-100 px-2.5 py-1 text-xs font-bold text-error-700 dark:bg-error-500/10 dark:text-error-400">
                                        <span class="h-1.5 w-1.5 rounded-full bg-error-500"></span>
                                        Disuspend
                                    </span>
                                @else
                                    <span
                                        class="inline-flex items-center gap-1 rounded-full bg-success-100 px-2.5 py-1 text-xs font-bold text-success-700 dark:bg-success-500/10 dark:text-success-400">
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center justify-end gap-1">
                                    ...
                                    @if (!$isSelf)
                                        {{-- Ubah Role --}}
                                        <button type="button" @click="activeModal = 'role-{{ $user->id }}'"
                                        ...
                                        @if (!$isSuperAdmin)
                                            ...
                                            @else
                                                <button type="button" @click="activeModal = 'suspend-{{ $user->id }}'"
                                        ...
                                        {{-- Reset Password --}}
                                        <button type="button" @click="activeModal = 'reset-{{ $user->id }}'"
                            ...
                            <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4"
                                ...
                                <div
                                    ...
                                    <p class="mb-4 text-sm text-gray-600 dark:text-gray-400">
                                        Mengubah role untuk <strong
                                            class="text-gray-800 dark:text-white">{{ $user->name }}</strong>.
                                    </p>
                                    <form method="POST" action="{{ route('admin.users.update-role', $user) }}">
                                        ...
                                        <div class="mb-4 space-y-2">
                                            @foreach ($roles as $role)
                                                <label
                                                    class="flex cursor-pointer items-center gap-3 rounded-xl border-2 border-gray-200 px-4 py-3 transition has-[:checked]:border-brand-500 has-[:checked]:bg-brand-50 dark:border-dark-border dark:has-[:checked]:bg-brand-500/10">
                            ...
                            <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4"
                                ...
                                <div
                                    ...
                                    <form method="POST" action="{{ route('admin.users.suspend', $user) }}">
                                        ...
                                        <div class="mb-4">
                                            <label
                                                class="mb-1.5 block text-xs font-semibold text-gray-600 dark:text-gray-400">Alasan
                                                Suspend (opsional)</label>
                        ...
                        <template x-if="activeModal === 'reset-{{ $user->id }}'">
                            <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4"
                                ...
                                <div class="w-full max-w-sm rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl dark:border-dark-border dark:bg-dark-card"
                                    ...
                                    <p class="mb-4 text-sm text-gray-600 dark:text-gray-400">
                                        Set password baru untuk <strong
                                            class="text-gray-800 dark:text-white">{{ $user->name }}</strong>.
                                    </p>
                                    <form method="POST" action="{{ route('admin.users.reset-password', $user) }}">
                                        ...
                                        <div class="mb-3">
                                            <label
                                                class="mb-1.5 block text-xs font-semibold text-gray-600 dark:text-gray-400">Password
                                                Baru</label>
                                            <div class="relative">
                                                ...
                                                <button type="button" @click="showPw = !showPw"
                                                    ...
                                                    <svg class="h-4 w-4" fill="none" stroke="currentColor"
                                                        viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            stroke-width="2" x-show="!showPw"
                                                            d="M15 12a3 3 0 11-6 0 3 3 0 016 0zM2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            stroke-width="2" x-show="showPw" x-cloak
                                        ...
                                        <div class="mb-4">
                                            <label
                                                class="mb-1.5 block text-xs font-semibold text-gray-600 dark:text-gray-400">Konfirmasi
                                                Password</label>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-5 py-12 text-center">
                                <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Tidak ada user ditemukan.
                                </p>
                            </td>
                        </tr>
                    @endforelse
