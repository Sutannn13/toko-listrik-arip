        ...
        <main class="flex-1 w-full mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
            ...
            @if ($cartItems->isEmpty())
                <section class="rounded-2xl border-2 border-dashed border-gray-200 bg-gray-50 p-12 text-center">
                    <div
                        class="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-white shadow-sm mb-4">
                ...
                <section class="grid gap-8 lg:grid-cols-[1.5fr,1fr]">
                            ...
                            <article
                                ...
                                <div class="flex flex-col gap-3 min-w-[200px]">
                                    ...
                                    <div class="flex justify-end gap-2">
                                        <form method="POST"
                                            action="{{ route('home.cart.update', $item['product_id']) }}"
                                        ...
                                        <form method="POST"
                                            action="{{ route('home.cart.remove', $item['product_id']) }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                    ...
                    <aside class="sticky top-24 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm self-start">
                        ...
                        <div class="mt-4 space-y-3">
                            ...
                            <div
                                class="mt-4 rounded-xl border border-blue-200 bg-blue-50 p-3 flex gap-3 text-sm text-blue-800">
                                <svg class="h-5 w-5 shrink-0 text-blue-600" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                </svg>
                                <p>Default garansi tiap item adalah <strong>7 hari</strong>, dan admin dapat
                                    menyesuaikan hingga maksimal <strong>30 hari</strong>.</p>
                            </div>
                        </div>
                                ...
                                <div class="grid gap-4 p-4 rounded-xl border border-gray-100 bg-gray-50">
                                    ...
                                    <div class="grid gap-3 pt-2">
                                        ...
                                        <div class="grid gap-3 grid-cols-3">
                                            ...
                                            <div class="col-span-1">
                                                <label
                                                    class="mb-1 block text-[10px] sm:text-xs font-semibold text-gray-600">Kode
                                                    Pos</label>
                                        ...
                                        <div>
                                            <label
                                                class="mb-1 block text-xs font-semibold text-gray-600">Catatan/Patokan</label>
                                            <input type="text" name="address_notes"
                                                value="{{ old('address_notes') }}"
                                                class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-primary-500 focus:ring-1 focus:ring-primary-500"
                                    ...
                                    @auth
                                        <label
                                            class="mt-2 inline-flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
            ...
            @if ($recentOrders->isNotEmpty())
                <section class="mt-12 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm sm:p-8">
                    <div class="mb-6">
                        ...
                        <p class="text-sm text-gray-500 mt-1">Cek status pesanan atau ajukan klaim garansi untuk produk
                            yang sudah dibeli.</p>
                    </div>
                            ...
                            <article class="rounded-xl border border-gray-200 bg-gray-50 overflow-hidden">
                                <div
                                    ...
                                    <div class="flex items-center gap-4">
                                        <div>
                                            <p
                                                class="text-xs font-semibold text-gray-500 uppercase tracking-widest mb-0.5">
                                                Order ID</p>
                                ...
                                <div class="p-5">
                                    <div class="mb-4 flex flex-wrap gap-x-8 gap-y-2">
                                        <p class="text-sm text-gray-600">Total Belanja: <span
                                                class="font-bold text-gray-900">Rp
                                                {{ number_format((int) $order->total_amount, 0, ',', '.') }}</span>
                                            ({{ number_format((int) $order->items->sum('quantity')) }} item)</p>
                                    ...
                                    <div class="space-y-3">
                                        @foreach ($order->items as $orderItem)
                                            @php
                                                ...
                                                $isOrderOwner =
                                                    auth()->check() && (int) $order->user_id === (int) auth()->id();
                                                $hasOpenClaim =
                                                    $latestClaim &&
                                                    in_array(
                                                        $latestClaim->status,
                                                ...
                                                $claimCount = $orderItem->warrantyClaims->count();
                                                $remainingClaim = max(0, 2 - $claimCount);
                                            @endphp
                                            ...
                                            <div
                                                ...
                                                <div class="flex-1">
                                                    ...
                                                    <p class="mt-1 text-[11px] text-gray-500">
                                                        Sisa klaim garansi: <span
                                                            class="font-semibold text-gray-700">{{ $remainingClaim }}
                                                            dari 2</span>
                                                    </p>
                                                </div>
                                                ...
                                                <div class="w-full sm:w-auto text-right">
                                                    ...
                                                    @auth
                                                        @if ($isOrderOwner && $warrantyActive && !$hasOpenClaim && $claimCount < 2)
                                                            <form method="POST"
                                                                ...
                                                                <div class="flex gap-2">
                                                                    ...
                                                                    <button type="submit"
                                                                        class="shrink-0 rounded-lg bg-gray-900 px-3 py-2 text-xs font-semibold text-white transition hover:bg-gray-800">
                                                                        Ajukan Klaim
                                                                    </button>
                                                                </div>
                                                            </form>
                                                        @elseif ($isOrderOwner && $hasOpenClaim)
                                                            <p class="mt-1 text-xs text-orange-600 font-medium">Sedang
                                                                diproses tim Arip.</p>
                                                        @elseif ($isOrderOwner && $claimCount >= 2)
                                                            <p class="mt-1 text-xs text-red-600 font-medium">Batas klaim 2x
                                                                sudah tercapai.</p>
