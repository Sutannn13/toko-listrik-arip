...
@section('content')
            ...
            <div class="flex flex-wrap items-center justify-between gap-4">
                ...
                <div class="flex flex-col items-end gap-2">
                    @php
                        ...
                        $statusLabels = [
                            'completed' => 'Selesai', 'cancelled' => 'Dibatalkan', 'shipped' => 'Dikirim',
                            'processing' => 'Diproses', 'pending' => 'Menunggu',
                        ];
                    @endphp
                    <span class="inline-flex items-center rounded-full border px-3 py-1 text-xs font-bold {{ $statusColors[$order->status] ?? $statusColors['pending'] }}">
                    ...
                    @php
                        ...
                        $payLabels = ['paid' => 'Lunas', 'pending' => 'Belum Bayar', 'failed' => 'Gagal'];
                    @endphp
                    <span class="inline-flex items-center rounded-full border px-3 py-1 text-xs font-bold {{ $payColors[$order->payment_status] ?? $payColors['pending'] }}">
            ...
            <div class="border-t border-gray-100 bg-gray-50/50 px-6 py-4">
                <div class="flex items-center">
                    @foreach ($stepperSteps as $i => $step)
                        @php $stepNum = $i + 1; $isActive = $stepNum <= $paymentStep; $isCurrent = $stepNum === $paymentStep; @endphp
                        <div class="flex items-center {{ !$loop->last ? 'flex-1' : '' }}">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="mt-1.5 text-[10px] font-semibold text-center {{ $isActive ? 'text-primary-700' : 'text-gray-400' }}">{{ $step['label'] }}</p>
                            </div>
                            @unless ($loop->last)
                                <div class="mx-2 flex-1 h-0.5 rounded {{ $stepNum < $paymentStep ? 'bg-primary-500' : 'bg-gray-200' }}"></div>
                            @endunless
                        </div>
                    @endforeach
                </div>
    ...
    @if ($order->tracking_number)
        <div class="mb-6 flex items-center justify-between rounded-xl border border-primary-200 bg-primary-50 p-4 shadow-sm">
            <div>
    ...
    @if ($order->status !== 'cancelled' && $order->payment_status !== 'paid')
        @if ($latestPayment)
            @if ($isBayarGgMethod)
                <div class="mb-6 rounded-2xl border border-indigo-200 bg-white shadow-sm overflow-hidden">
                    ...
                    <div class="p-5 space-y-4">
                        <p class="text-sm text-gray-600">Scan QRIS dari halaman Bayar.gg — verifikasi otomatis tanpa upload bukti.</p>
                        <div class="grid gap-2 rounded-xl border border-gray-200 bg-gray-50 p-4 text-sm text-gray-700">
                            <p><span class="font-semibold text-gray-900">Invoice:</span> {{ $latestPayment->gateway_invoice_id ?: '-' }}</p>
                            <p><span class="font-semibold text-gray-900">Status:</span> {{ strtoupper($latestPayment->gateway_status ?: 'pending') }}</p>
                            @if ($latestPayment->gateway_expires_at)
                                <p><span class="font-semibold text-gray-900">Batas Waktu:</span> {{ $latestPayment->gateway_expires_at->format('d M Y H:i') }}</p>
                            @endif
                        </div>
                        <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                            ...
                            <form action="{{ route('home.tracking.bayargg.regenerate', $order->order_code) }}" method="POST">
                                @csrf
                                <button type="submit" class="inline-flex items-center justify-center rounded-xl border border-indigo-300 bg-white px-5 py-3 text-xs font-semibold text-indigo-700 transition hover:bg-indigo-50">
                                    Buat Link Ulang
                                </button>
                            </form>
                        </div>
                ...
                <div class="mb-6 rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                    ...
                    <div class="p-5 space-y-4">
                        <p class="text-sm text-gray-600">
                            Transfer <strong class="text-gray-900">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</strong> lalu upload bukti agar status masuk antrean verifikasi admin.
                        </p>
                            ...
                            <div class="grid gap-3 sm:grid-cols-2">
                                @forelse ($bankTransferAccounts as $account)
                                    <div class="rounded-xl border border-gray-200 bg-gray-50 p-4">
                                        <p class="text-xs font-bold text-gray-500 uppercase tracking-wider">{{ $account['name'] }}</p>
                                        <div class="mt-2 flex items-center justify-between gap-2">
                                            <p class="text-lg font-bold text-gray-900 font-mono tracking-wide">{{ $account['number'] }}</p>
                                            <button type="button" onclick="navigator.clipboard.writeText('{{ $account['number'] }}')"
                                                    class="shrink-0 rounded-lg border border-gray-300 bg-white p-2 text-gray-500 transition hover:bg-gray-100 hover:text-gray-700" title="Salin nomor rekening">
                                ...
                                @empty
                                    <p class="text-xs text-gray-500 col-span-full">Info rekening bank belum disetel oleh admin.</p>
                                @endforelse
                            </div>
                        @elseif($latestPayment->method === 'ewallet')
                            <div class="rounded-xl border border-gray-200 bg-gray-50 p-4">
                                <p class="text-xs font-bold text-gray-500 uppercase tracking-wider">{{ $ewalletAccount['name'] }}</p>
                                <div class="mt-2 flex items-center justify-between gap-2">
                                    <p class="text-lg font-bold text-gray-900 font-mono tracking-wide">{{ $ewalletAccount['number'] ?: '-' }}</p>
                                    @if ($ewalletAccount['number'])
                                        <button type="button" onclick="navigator.clipboard.writeText('{{ $ewalletAccount['number'] }}')"
                                                class="shrink-0 rounded-lg border border-gray-300 bg-white p-2 text-gray-500 transition hover:bg-gray-100" title="Salin">
                                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
                                        </button>
                                    @endif
                                </div>
                        ...
                        <form action="{{ route('home.tracking.proof', $order->order_code) }}" method="POST"
                              ...
                              class="space-y-3">
                            @csrf
                            <label class="group flex cursor-pointer flex-col items-center rounded-xl border-2 border-dashed border-gray-300 bg-gray-50/50 p-6 transition hover:border-primary-400 hover:bg-primary-50/30">
                                <div x-show="!preview" class="text-center">
                                    <svg class="mx-auto h-10 w-10 text-gray-400 group-hover:text-primary-500 transition" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                                    </svg>
                                    <p class="mt-2 text-sm font-semibold text-gray-700">Drag & drop atau klik untuk upload</p>
                                    <p class="mt-1 text-xs text-gray-500">JPG, PNG — Maks 2MB</p>
                                </div>
                                <img x-show="preview" :src="preview" class="max-h-32 rounded-lg object-contain" alt="Preview">
                                <p x-show="fileName" x-text="fileName" class="mt-2 text-xs font-medium text-primary-700"></p>
            ...
            @elseif($isProofPaymentMethod && $latestPayment->proof_url)
                ...
                <div class="mb-6 rounded-2xl border {{ $isProofRejected ? 'border-red-200' : 'border-emerald-200' }} bg-white shadow-sm overflow-hidden">
                    <div class="px-5 py-4 {{ $isProofRejected ? 'bg-red-50 border-b border-red-100' : 'bg-emerald-50 border-b border-emerald-100' }}">
                    ...
                    <div class="p-5 space-y-4">
                        @if ($isProofRejected && $latestPayment->notes)
                            <div class="rounded-lg border border-red-200 bg-red-50 px-4 py-3">
                                <p class="text-[11px] font-bold uppercase tracking-wider text-red-700 mb-1">Catatan Admin</p>
                                <p class="text-sm text-red-700">{{ $latestPayment->notes }}</p>
                            </div>
                        @endif

                        <a href="{{ Storage::disk('public')->url($latestPayment->proof_url) }}" target="_blank" rel="noopener noreferrer"
                        ...
                        @if (in_array($latestPayment->status, ['pending', 'failed'], true))
                            <form action="{{ route('home.tracking.proof', $order->order_code) }}" method="POST"
                                  enctype="multipart/form-data" x-data="{ fileName: '', preview: null }" class="space-y-3 pt-3 border-t border-gray-100">
                                @csrf
                                <input type="hidden" name="replace_proof" value="1">
                                <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Ganti Bukti Pembayaran</p>
                                <label class="group flex cursor-pointer flex-col items-center rounded-xl border-2 border-dashed border-gray-300 bg-gray-50/50 p-5 transition hover:border-primary-400 hover:bg-primary-50/30">
                                    <div x-show="!preview" class="text-center">
                                        <svg class="mx-auto h-8 w-8 text-gray-400 group-hover:text-primary-500 transition" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                                        </svg>
                                        <p class="mt-2 text-xs font-medium text-gray-600">Klik atau drag file baru</p>
                                    </div>
                                    <img x-show="preview" :src="preview" class="max-h-24 rounded-lg object-contain" alt="Preview">
                                    <p x-show="fileName" x-text="fileName" class="mt-1 text-xs font-medium text-primary-700"></p>
                                    <input type="file" name="payment_proof" accept="image/*" required class="sr-only"
                                           @change="fileName = $event.target.files[0]?.name || ''; if($event.target.files[0]) { const reader = new FileReader(); reader.onload = (e) => preview = e.target.result; reader.readAsDataURL($event.target.files[0]); }">
                                </label>
                                @error('payment_proof')
                                    <p class="text-xs text-red-600">{{ $message }}</p>
                                @enderror
                                <button type="submit" class="w-full rounded-xl {{ $isProofRejected ? 'bg-red-600 hover:bg-red-700' : 'bg-primary-600 hover:bg-primary-700' }} px-6 py-3 text-sm font-bold text-white shadow-sm transition">
                                    Ganti Bukti
                                </button>
                            </form>
                        @endif
            ...
            @elseif($isCodMethod)
                <div class="mb-6 rounded-2xl border border-emerald-200 bg-white shadow-sm overflow-hidden">
                    ...
                    <div class="p-5">
                        <p class="text-sm text-gray-600">Tidak perlu upload bukti pembayaran. Siapkan uang pas saat kurir tiba di alamat Anda.</p>
                    </div>
                </div>
            @endif
        @endif
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        ...
        <div class="lg:col-span-1 space-y-6">
            <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                <h3 class="text-xs font-bold text-gray-500 uppercase tracking-wider border-b border-gray-100 pb-3 mb-4">Informasi Pengiriman</h3>
                <div class="space-y-3">
                    <div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white shadow-sm flex flex-col h-full overflow-hidden">
                    ...
                    @foreach ($order->items as $item)
                        <div class="flex items-center gap-4 px-5 py-4">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-gray-500 mt-0.5">{{ number_format($item->quantity) }} × Rp {{ number_format($item->price, 0, ',', '.') }}</p>
                            </div>
                ...
                <div class="border-t border-gray-200 px-5 py-4 bg-gray-50/50 space-y-2">
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-600">Subtotal ({{ number_format($order->items->sum('quantity')) }} item)</span>
