...
@section('content')
    <form action="{{ route('admin.settings.update') }}" method="POST" x-data="{ activeTab: 'store' }">
        ...
        {{-- Tab Bar --}}
        <div
            class="mb-6 flex flex-wrap gap-1 rounded-2xl border border-gray-200 bg-white p-1.5 shadow-sm dark:border-dark-border dark:bg-dark-card">
            @php
                $tabs = [
                    'store' => [
                        'label' => 'Info Toko',
        ...
        <div x-show="activeTab === 'store'" x-cloak>
            <div
                ...
                <div class="mt-8 border-t border-gray-100 pt-6 dark:border-dark-border">
                    <h3 class="text-sm font-bold uppercase tracking-wider text-gray-600 dark:text-gray-300">Social Media
                    </h3>
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">URL profil sosial yang tampil di footer
                        website.</p>
        ...
        <div x-show="activeTab === 'general'" x-cloak>
            <div
                ...
                <p class="mb-6 text-sm text-gray-500 dark:text-gray-400">Aktifkan untuk menutup toko sementara dari publik.
                </p>

                @if (isset($settings['maintenance_mode']))
                    <div
                        class="mb-6 rounded-xl border border-gray-100 bg-gray-50 p-5 dark:border-dark-border dark:bg-dark-hover">
                        <label class="flex cursor-pointer items-start gap-4">
                            <div class="relative mt-0.5">
                                <input type="hidden" name="maintenance_mode" value="0">
                                <input type="checkbox" name="maintenance_mode" value="1" id="maintenance_mode"
                                    class="sr-only peer" @if ($settings['maintenance_mode']->value === '1') checked @endif>
                                <div
                                    class="h-6 w-11 rounded-full bg-gray-300 peer-checked:bg-error-500 transition-colors dark:bg-gray-600 peer-checked:dark:bg-error-500">
                                </div>
                                <div
                            ...
                            <div>
                                ...
                                <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">Pengunjung akan melihat halaman
                                    maintenance, admin tetap bisa masuk.</p>
                            </div>
                        </label>
                    </div>
                @endif
                ...
                <div
                    class="mt-4 rounded-xl border border-warning-200 bg-warning-50 p-4 flex gap-3 dark:border-warning-500/20 dark:bg-warning-500/10">
                    <svg class="h-5 w-5 shrink-0 text-warning-500 mt-0.5" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z" />
                    </svg>
                    <p class="text-sm text-warning-800 dark:text-warning-300">
                        <strong>Perhatian:</strong> Mode maintenance akan memblokir seluruh akses pelanggan. Pastikan
        ...
        <div x-show="activeTab === 'bank'" x-cloak>
            <div
                class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-dark-border dark:bg-dark-card">
                <h2 class="mb-1 text-base font-bold text-gray-800 dark:text-white">Akun Pembayaran</h2>
                <p class="mb-6 text-sm text-gray-500 dark:text-gray-400">Kelola akun transfer bank (BCA/BRI) dan
                    e-wallet/e-money yang ditampilkan saat checkout.</p>

                <p class="mb-3 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-300">Transfer
                    Bank (BCA/BRI)</p>
                @foreach ([1, 2] as $n)
                    <div
                        class="mb-6 rounded-xl border border-gray-100 bg-gray-50 p-5 dark:border-dark-border dark:bg-dark-hover">
                ...
                <p class="mb-3 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-300">E-Wallet /
                    E-Money (DANA)</p>
                <div class="rounded-xl border border-gray-100 bg-gray-50 p-5 dark:border-dark-border dark:bg-dark-hover">
                    <div class="grid gap-4 sm:grid-cols-3">
                        @foreach (['bank_3_name', 'bank_3_account', 'bank_3_holder'] as $key)
                            @if (isset($settings[$key]))
                                <x-admin.setting-input :setting="$settings[$key]" />
                            @endif
                        @endforeach
        ...
        <div x-show="activeTab === 'hours'" x-cloak>
            <div
                ...
                <p class="mb-6 text-sm text-gray-500 dark:text-gray-400">Jadwal buka & tutup toko yang ditampilkan di
                    halaman utama.</p>
                ...
                <div
                    class="mt-5 rounded-xl border border-blue-100 bg-blue-50 p-4 dark:border-blue-500/20 dark:bg-blue-500/10">
                    <p class="text-sm text-blue-800 dark:text-blue-300">
                        💡 Format jam: <code class="rounded bg-blue-100 px-1.5 py-0.5 text-xs dark:bg-blue-900/30">08:00 -
                            17:00</code>
                        atau ketik <code class="rounded bg-blue-100 px-1.5 py-0.5 text-xs dark:bg-blue-900/30">Tutup</code>
                        jika libur.
                    </p>
                </div>
            </div>
                        ...
                        @if (isset($settings[$key]))
                            ...
                            <label
                                ...
                                <div class="relative shrink-0">
                                    ...
                                    <div
                                        class="h-6 w-11 rounded-full bg-gray-300 peer-checked:bg-brand-500 transition-colors dark:bg-gray-600">
                                    </div>
                                    <div
                                        class="absolute left-0.5 top-0.5 h-5 w-5 rounded-full bg-white shadow-sm transition-transform peer-checked:translate-x-5">
                                    </div>
