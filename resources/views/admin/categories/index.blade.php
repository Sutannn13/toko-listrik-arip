...
@section('content')
    ...
    <p class="mb-2 text-xs font-medium text-gray-500 sm:hidden">Geser tabel ke samping untuk melihat semua kolom termasuk
        aksi.</p>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-dark-border">
                    @forelse($categories as $i => $category)
                        <tr class="transition hover:bg-gray-50/50 dark:hover:bg-dark-hover/50">
                            ...
                            <td class="px-5 py-4">
                                <code
                                    class="rounded bg-gray-100 px-2 py-0.5 text-xs text-gray-600 dark:bg-dark-hover dark:text-gray-400">
                                    {{ $category->slug }}
                                </code>
                            </td>
                            <td class="px-5 py-4">
                            ...
                            <td class="px-5 py-4 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    ...
                                    @php
                                        $categoryHasProducts = $category->products_count > 0;
                                        $categoryDeleteTitle = 'Konfirmasi Penghapusan Kategori';
                                        $categoryDeleteMessage = $categoryHasProducts
                                            ? 'Kategori "' .
                                                $category->name .
                                                '" akan dihapus bersama seluruh produk di dalamnya.'
                                            : 'Kategori "' . $category->name . '" akan dihapus dari master katalog.';
                                        $categoryDeleteImpact = $categoryHasProducts
                                            ? $category->products_count .
                                                ' produk terkait kategori ini akan ikut terhapus. Riwayat pesanan pelanggan tetap tersimpan.'
                                            : 'Tidak ada produk aktif di kategori ini, sehingga tidak ada item produk yang terdampak.';
                                    @endphp
                                    <form method="POST" action="{{ route('admin.categories.destroy', $category) }}"
                                        @submit.prevent="askFormConfirmation($el, {
                                            title: @js($categoryDeleteTitle),
                                            context: 'Perubahan ini memengaruhi struktur katalog dan pelaporan produk.',
                                            message: @js($categoryDeleteMessage),
                                            impact: @js($categoryDeleteImpact),
                                            confirmText: 'Hapus Kategori',
                                            cancelText: 'Tinjau Ulang',
                                            tone: 'danger',
                                            note: 'Lanjutkan hanya jika validasi data kategori sudah selesai dilakukan.',
                                        })">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            class="rounded-lg border border-gray-200 bg-white p-1.5 text-gray-500 transition dark:border-dark-border dark:bg-dark-card dark:text-gray-400 {{ $categoryHasProducts ? 'hover:border-warning-300 hover:bg-warning-50 hover:text-warning-600' : 'hover:border-error-300 hover:bg-error-50 hover:text-error-600' }}"
                                            title="{{ $categoryHasProducts ? 'Hapus kategori beserta produknya' : 'Hapus Kategori' }}">
                                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
