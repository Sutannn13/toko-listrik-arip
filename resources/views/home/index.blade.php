...
@php
    ...
    $categoryIcons = [
        'lampu' => '<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/></svg>',
        'kabel' => '<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>',
        'saklar' => '<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"/></svg>',
        'mcb' => '<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>',
        'default' => '<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg>',
    ];
...
@section('content')
    @if (session('success'))
        <div x-data="{ show: true }" x-show="show" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 -translate-y-2"
            ...
            <button @click="show = false" class="shrink-0 rounded-md p-0.5 hover:bg-green-100"><svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
        </div>
    @endif
    ...
    @if (session('error'))
        <div x-data="{ show: true }" x-show="show" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
            ...
            <button @click="show = false" class="shrink-0 rounded-md p-0.5 hover:bg-red-100"><svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
        </div>
    @endif
    ...
    <div x-data="heroBannerCarousel()" x-init="start()" class="mb-6 sm:mb-8">
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary-600 via-primary-500 to-teal-500 shadow-lg shadow-primary-500/20">
            <!-- Decorative elements -->
            ...
            <div class="relative h-[160px] sm:h-[180px] md:h-[200px]">
                <!-- Slide 1: Main Store -->
                <div x-show="active === 0"
                    x-transition:enter="transition duration-500 ease-out" x-transition:enter-start="translate-x-8 opacity-0" x-transition:enter-end="translate-x-0 opacity-100"
                    x-transition:leave="transition duration-400 ease-in" x-transition:leave-start="translate-x-0 opacity-100" x-transition:leave-end="-translate-x-8 opacity-0"
                    class="absolute inset-0 flex items-center px-6 sm:px-10 md:px-14">
                    <div class="flex-1">
                        <p class="text-xs font-bold uppercase tracking-widest text-white/70 sm:text-sm">{{ $storeTagline ?: '⚡ Toko Listrik Terpercaya' }}</p>
                        <h2 class="mt-1 text-xl font-black text-white sm:text-2xl md:text-3xl">Pusat Kebutuhan<br>Listrik Anda</h2>
                        <p class="mt-2 text-xs text-white/80 sm:text-sm max-w-md">{{ $totalProducts }} produk tersedia dari {{ $totalCategories }} kategori dengan harga transparan dan garansi resmi.</p>
                        <a href="#catalog-section" class="mt-3 inline-flex items-center gap-2 rounded-full bg-white px-4 py-2 text-xs font-bold text-primary-700 shadow-md transition hover:bg-primary-50 sm:text-sm">
                            Jelajahi Katalog
                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
                        </a>
                    </div>
                    <div class="hidden sm:flex flex-col items-center gap-3">
                        <div class="flex h-16 w-16 md:h-20 md:w-20 items-center justify-center rounded-2xl bg-white/20 backdrop-blur-sm">
                            <svg class="h-10 w-10 md:h-12 md:w-12 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
                        </div>
                    </div>
                </div>
                ...
                <div x-show="active === 1"
                    ...
                    class="absolute inset-0 flex items-center px-6 sm:px-10 md:px-14">
                    <div class="flex-1">
                        <p class="text-xs font-bold uppercase tracking-widest text-white/70 sm:text-sm">🛡️ Garansi Resmi</p>
                        <h2 class="mt-1 text-xl font-black text-white sm:text-2xl md:text-3xl">Garansi 7 Hari<br>Produk Elektronik</h2>
                        <p class="mt-2 text-xs text-white/80 sm:text-sm max-w-md">Upload bukti kerusakan, tim admin memproses klaim Anda dalam 48 jam.</p>
                        <a href="{{ route('home.warranty') }}" class="mt-3 inline-flex items-center gap-2 rounded-full bg-white px-4 py-2 text-xs font-bold text-primary-700 shadow-md transition hover:bg-primary-50 sm:text-sm">
                            Klaim Garansi
                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
                        </a>
                    </div>
                ...
                <div x-show="active === 2"
                    ...
                    <div class="flex-1">
                        ...
                        <h2 class="mt-1 text-xl font-black text-white sm:text-2xl md:text-3xl">Bayar di Tempat<br>COD Tersedia</h2>
                        <p class="mt-2 text-xs text-white/80 sm:text-sm max-w-md">Pilih COD saat checkout, bayar saat barang sampai di rumah. Zero ribet!</p>
                        <a href="#catalog-section" class="mt-3 inline-flex items-center gap-2 rounded-full bg-white px-4 py-2 text-xs font-bold text-primary-700 shadow-md transition hover:bg-primary-50 sm:text-sm">
                            Belanja Sekarang
                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
                        </a>
    ...
    <div class="mb-6 sm:mb-8 grid grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-4">
        <div class="flex items-center gap-3 rounded-xl border border-gray-100 bg-white p-3 sm:p-4 shadow-sm">
            ...
            <div class="min-w-0">
                <p class="text-xs font-bold text-gray-900 sm:text-sm">Garansi 7 Hari</p>
                <p class="text-[10px] text-gray-500 sm:text-xs">Produk Elektronik</p>
            </div>
        </div>
        <div class="flex items-center gap-3 rounded-xl border border-gray-100 bg-white p-3 sm:p-4 shadow-sm">
            <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-blue-50 text-blue-600">
                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
            </div>
            <div class="min-w-0">
    ...
    @if ($categories->count() > 0)
        <div class="mb-6 sm:mb-8">
            ...
            <div x-data="{ categoryOpen: false }" class="relative">
                <button type="button" @click="categoryOpen = !categoryOpen" @keydown.escape.window="categoryOpen = false"
                    ...
                    <span class="flex items-center gap-2.5">
                        @if ($activeCategory)
                            ...
                            <span class="rounded-full bg-primary-100 px-2 py-0.5 text-[10px] font-bold text-primary-600">{{ $activeCategory->active_products_count }} produk</span>
                        @else
                            ...
                            <span class="rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-bold text-gray-500">{{ $totalProducts }} produk</span>
                        @endif
                    </span>
                    <svg class="h-5 w-5 text-gray-400 transition" :class="categoryOpen ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <div x-cloak x-show="categoryOpen" @click.away="categoryOpen = false"
                    ...
                    <div class="max-h-64 overflow-y-auto">
                        ...
                        <a href="{{ route('home', array_filter(['q' => $keyword])) }}"
                            ...
                            <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg {{ !$activeCategory ? 'bg-primary-200 text-primary-700' : 'bg-gray-100 text-gray-500' }}">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zm10 0a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zm10 0a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"/></svg>
                            </span>
                            <span class="flex-1">Semua Kategori</span>
                            <span class="rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-bold {{ !$activeCategory ? 'bg-primary-100 text-primary-600' : 'text-gray-500' }}">{{ $totalProducts }}</span>
                            @if (!$activeCategory)
                        ...
                        @foreach ($categories as $category)
                            ...
                            <a href="{{ route('home', array_filter(['q' => $keyword, 'category' => $category->id])) }}"
                                ...
                                <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg {{ $isActive ? 'bg-primary-200 text-primary-700' : 'bg-gray-100 text-gray-500' }}">
                                    {!! $getCategoryIcon($category->name) !!}
                                </span>
                                <span class="flex-1">{{ $category->name }}</span>
                                <span class="rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-bold {{ $isActive ? 'bg-primary-100 text-primary-600' : 'text-gray-500' }}">{{ $category->active_products_count }}</span>
                                @if ($isActive)
                                    <svg class="h-4 w-4 text-primary-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
                                @endif
                            </a>
    ...
    <div id="catalog-section" class="mb-6 sm:mb-8">
        <form method="GET" action="{{ route('home') }}" class="flex gap-2">
            <div class="relative flex-1">
                <svg class="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button type="submit"
                ...
                <svg class="h-5 w-5 sm:hidden" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                <span class="hidden sm:inline">Cari Produk</span>
            </button>
            @if ($keyword !== '' || $activeCategory)
                <a href="{{ route('home') }}" class="shrink-0 flex items-center justify-center rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm font-semibold text-gray-600 shadow-sm transition hover:bg-gray-50 active:scale-95">
        ...
        @if ($keyword !== '' || $activeCategory)
            <div class="mt-2 flex flex-wrap items-center gap-2 text-xs text-gray-500">
                ...
                @if ($keyword !== '')
                    <span class="inline-flex items-center gap-1 rounded-full bg-gray-100 px-2.5 py-1 font-medium text-gray-700">
                        "{{ $keyword }}"
                        <a href="{{ route('home', $activeCategory ? ['category' => $activeCategory->id] : []) }}" class="ml-0.5 text-gray-400 hover:text-gray-600">&times;</a>
                    </span>
                @endif
                @if ($activeCategory)
                    <span class="inline-flex items-center gap-1 rounded-full bg-primary-50 px-2.5 py-1 font-medium text-primary-700">
                        {{ $activeCategory->name }}
                        <a href="{{ route('home', $keyword !== '' ? ['q' => $keyword] : []) }}" class="ml-0.5 text-primary-400 hover:text-primary-600">&times;</a>
                    </span>
                @endif
            </div>
        @endif
    ...
    <section>
        <div class="mb-4 flex items-center justify-between">
            ...
            <span class="rounded-full bg-gray-100 px-3 py-1 text-xs font-semibold text-gray-600">{{ $products->total() }} produk</span>
        </div>
        ...
        @forelse ($products as $product)
            ...
            <article class="group flex flex-col rounded-2xl border border-gray-200 bg-white shadow-sm transition duration-200 hover:-translate-y-0.5 hover:border-primary-200 hover:shadow-lg overflow-hidden">
                <a href="{{ route('home.products.show', $product->slug) }}" class="block overflow-hidden bg-gray-50 aspect-square relative">
                    ...
                    <div class="absolute top-2 left-2 flex flex-col gap-1">
                        @if ($isBestseller)
                            <span class="inline-flex items-center gap-0.5 rounded-md bg-amber-500 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wide text-white shadow-sm">
                        ...
                        @endif
                        @if ($isLowStock)
                            <span class="inline-flex items-center rounded-md bg-red-500 px-1.5 py-0.5 text-[9px] font-bold uppercase text-white shadow-sm">
                                Stok Terbatas
                            </span>
                        @endif
                    </div>
                    ...
                    @if ($product->stock < 1)
                        <div class="absolute inset-0 flex items-center justify-center bg-gray-900/50 backdrop-blur-[1px]">
                            <span class="rounded-lg bg-white/90 px-3 py-1.5 text-xs font-bold text-gray-700">Stok Habis</span>
                        </div>
                    @endif
                </a>

                <div class="flex flex-1 flex-col p-3">
                    <h3 class="text-xs font-semibold text-gray-900 leading-snug line-clamp-2 group-hover:text-primary-700 sm:text-sm">
                        {{ $product->name }}
                    </h3>

                    {{-- Category tag --}}
                    <p class="mt-0.5 text-[10px] text-gray-400 font-medium">{{ $product->category->name ?? 'Uncategorized' }}</p>
                    ...
                    @if ($reviewCount > 0)
                        <div class="mt-1.5 flex items-center gap-1">
                            <div class="flex items-center">
                                @for ($i = 1; $i <= 5; $i++)
                                    <svg class="h-3 w-3 {{ $i <= round($avgRating) ? 'text-amber-400' : 'text-gray-200' }}" fill="currentColor" viewBox="0 0 20 20">
        ...
        @empty
            <div class="rounded-2xl border-2 border-dashed border-gray-200 bg-gray-50 p-12 text-center">
                ...
                <a href="{{ route('home') }}" class="mt-4 inline-flex items-center gap-2 rounded-xl bg-primary-600 px-5 py-2.5 text-sm font-bold text-white shadow-md shadow-primary-500/20 transition hover:bg-primary-700">
                    Lihat Semua Produk
                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
                </a>
            </div>
