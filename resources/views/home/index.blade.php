...
<body class="min-h-screen bg-gray-50 font-sans text-gray-800 antialiased selection:bg-primary-500 selection:text-white">
    <div class="relative z-10 flex min-h-screen flex-col">
        ...
        <main class="flex-1 w-full mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
            ...
            <!-- Page Header -->
            <section class="mb-8 rounded-2xl border border-gray-100 bg-white p-6 shadow-sm sm:p-8 flex flex-col md:flex-row gap-6 md:items-center justify-between">
                <div>
                    <span class="mb-2 inline-flex items-center rounded-full bg-primary-50 px-3 py-1 text-xs font-semibold text-primary-600">Katalog Resmi</span>
                    <h1 class="text-2xl font-extrabold text-gray-900 sm:text-3xl">Pusat Kebutuhan Listrik</h1>
                    <p class="mt-2 max-w-2xl text-sm text-gray-600">Jelajahi ratusan produk alat listrik dengan harga transparan dan stok riil.</p>
                </div>
            ...
            <section class="mb-8 rounded-2xl border border-gray-100 bg-white p-5 shadow-sm sm:p-6">
                ...
                <form method="GET" action="{{ route('home') }}" class="grid gap-3 lg:grid-cols-[1.5fr,1fr,auto,auto]">
                    <div class="relative">
                        ...
                        <input type="text" name="q" value="{{ $keyword }}" class="w-full pl-10 rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-900 focus:border-primary-500 focus:ring-1 focus:ring-primary-500" placeholder="Cari nama produk, kabel, saklar...">
                    </div>

                    <select name="category" class="w-full rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-900 focus:border-primary-500 focus:ring-1 focus:ring-primary-500">
                    ...
                    <button type="submit" class="rounded-xl w-full sm:w-auto bg-primary-600 px-6 py-2.5 text-sm font-bold text-white shadow-md shadow-primary-500/20 transition hover:bg-primary-700">
                        Cari Produk
                    </button>

                    <a href="{{ route('home') }}" class="rounded-xl w-full sm:w-auto border border-gray-300 bg-white px-6 py-2.5 text-center text-sm font-semibold text-gray-700 transition hover:bg-gray-50 flex items-center justify-center">
                        Reset
                    </a>
                </form>

                <div class="mt-5 flex flex-wrap gap-2 pt-4 border-t border-gray-100">
                    <a href="{{ route('home', $baseSearchQuery) }}" class="rounded-lg px-3 py-1.5 text-xs font-semibold transition {{ request('category') ? 'border border-gray-200 text-gray-600 hover:bg-gray-50' : 'bg-primary-100 text-primary-700' }}">
                        Semua Produk
                    </a>
                    @foreach ($categories as $category)
                        <a href="{{ route('home', array_filter(['q' => $keyword, 'category' => $category->id])) }}" class="rounded-lg px-3 py-1.5 text-xs font-semibold transition {{ (int) request('category') === $category->id ? 'bg-primary-100 text-primary-700' : 'border border-gray-200 text-gray-600 hover:bg-gray-50' }}">
            ...
            <section>
                ...
                @forelse ($products as $product)
                    ...
                    <article class="group flex flex-col rounded-2xl border border-gray-200 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:border-primary-200 hover:shadow-lg">
                        <div class="mb-3 flex items-start justify-between gap-3">
                            <div>
                                <h3 class="text-base font-bold text-gray-900 group-hover:text-primary-700">{{ $product->name }}</h3>
                                <p class="mt-0.5 text-xs text-gray-500">{{ $product->category->name ?? 'Uncategorized' }}</p>
                            </div>
                            <span class="whitespace-nowrap rounded font-bold bg-gray-100 px-2 py-1 text-[10px] uppercase tracking-wider text-gray-600">{{ $product->unit }}</span>
                        </div>
                        ...
                        <div class="mt-auto pt-4 flex flex-col gap-3">
                            <div class="flex items-center justify-between border-t border-gray-100 pt-3">
                                ...
                                @if($product->stock > 0)
                                    <p class="text-xs font-semibold text-green-600 bg-green-50 px-2 py-1 rounded">Sisa: {{ number_format($product->stock) }}</p>
                                @else
                            ...
                            <div class="flex items-center gap-2">
                                <a href="{{ route('home.products.show', $product->slug) }}" class="inline-flex w-1/3 items-center justify-center rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm font-semibold text-gray-700 transition hover:bg-gray-50 hover:text-primary-600">
                                    Detail
                                </a>
