...
@section('content')
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
            <h3 class="text-lg font-bold text-gray-800">Daftar Klaim Garansi</h3>
            <p class="text-sm text-gray-600">Tinjau, approve, reject, atau resolve klaim garansi user.</p>
        </div>
            ...
            <tbody class="text-gray-700">
                @forelse ($claims as $claim)
                    <tr class="border-b border-gray-200 align-top hover:bg-gray-50">
                        ...
                        <td class="p-4">
                            <p class="font-semibold">{{ $claim->orderItem->product_name ?? 'Item tidak ditemukan' }}</p>
                            <p class="text-xs text-gray-500">Qty: {{ $claim->orderItem->quantity ?? '-' }}</p>
                        </td>
                        <td class="p-4 text-sm text-gray-600">{{ $claim->reason }}</td>
                        <td class="p-4">
                            <span
                        ...
                        <td class="p-4">
                            ...
                            <form action="{{ route('admin.warranty-claims.update-status', $claim) }}" method="POST"
                                ...
                                <textarea name="admin_notes" rows="2"
                                    class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500"
                                    placeholder="Catatan admin...">{{ $claim->admin_notes }}</textarea>
                                <button type="submit"
                ...
                @empty
                    <tr>
                        <td colspan="6" class="p-6 text-center text-gray-500 italic">Belum ada klaim garansi masuk.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
